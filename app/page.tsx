...
const routeCities: DepartureCity[] = [
  "Одеса",
  "Дніпро",
  "Київ",
  "Харків",
...
const highlightedRoutesByCity: Record<DepartureCity, string[]> = {
  Дніпро: ["Дніпро → Кишинів", "Дніпро → Бухарест", "Дніпро → Ясси"],
  Одеса: ["Одеса → Кишинів", "Одеса → Бухарест", "Одеса → Ясси"],
  Київ: ["Київ → Кишинів", "Київ → Варшава", "Київ → Бухарест"],
  Харків: ["Харків → Кишинів", "Харків → Бухарест", "Харків → Ясси"],
  Львів: ["Львів → Кишинів", "Львів → Варшава", "Львів → Бухарест"],
  Кишинів: ["Кишинів → Одеса", "Кишинів → Київ", "Кишинів → Дніпро"]
};

const routeCitySourceLabels: Record<DepartureCity, string> = {
  Дніпро: "Дніпра",
  Одеса: "Одеси",
  Київ: "Києва",
  Харків: "Харкова",
...
export default function Home() {
  ...
  const [activeRouteCity, setActiveRouteCity] =
    useState<DepartureCity>("Одеса");
  const [showAllRoutes, setShowAllRoutes] = useState(false);
  const [isMobileRoutes, setIsMobileRoutes] = useState(false);
  const [homepageRoutes, setHomepageRoutes] = useState<HomepageRoute[]>([]);
  ...
  useEffect(() => {
    ...
    function updateViewportState(event?: MediaQueryListEvent) {
      setIsMobileRoutes(event ? event.matches : mediaQuery.matches);
    }
  ...
  const visibleRouteLimit = isMobileRoutes ? 8 : 10;
        ...
        <div className="mx-auto max-w-[1536px] px-4 pt-5 sm:px-6 md:px-8 lg:px-10 xl:px-12 2xl:px-14">
          <header className="header-shell relative z-30 rounded-[24px] px-[18px] py-3 sm:px-5 md:rounded-[30px] md:px-7 lg:px-[34px]">
            <div className="flex min-h-[72px] items-center justify-between gap-3 md:min-h-[74px] lg:grid lg:min-h-[88px] lg:grid-cols-[190px_1fr_300px] lg:justify-normal lg:gap-4 xl:grid-cols-[202px_1fr_310px]">
              <a href="#" className="header-brand block">
            ...
            <div className="all-routes-shell rounded-[30px] px-5 py-6 sm:px-7 md:px-9 md:py-8 lg:px-12 lg:py-10">
              ...
              <div className="mt-7 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                <p className="md:col-span-2 xl:col-span-3 text-[0.76rem] font-bold uppercase tracking-[0.18em] text-[var(--champagne)]">
                  Популярні напрямки з {routeCitySourceLabels[activeRouteCity]}:
                </p>
                {!routesLoaded ? (
                      ...
                      return (
                        <div
                          ...
                          className={`all-route-chip flex items-center justify-between gap-3 opacity-70 ${
                            highlightedRoutesByCity[activeRouteCity].includes(routeLabel)
                              ? "is-highlighted"
                              : ""
                          }`}
                        >
